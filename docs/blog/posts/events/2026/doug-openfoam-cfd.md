---
date: 2026-02-01
start: 2026-02-06T09:00:00
end: 2026-02-06T17:30:00
hero:
  backdrop: blog/events/2026/doug-openfoam-cfd.png
title: "Interested in CFD with OpenFOAM?"
description: "Join the mext Dutch OpenFOAM User Group meeting"
categories: [ Training ]
speakers: [ ]
price: 0.00
location: TU/e Campus, Pendulum 0.36
image: blog/events/2026/doug-openfoam-cfd.png
type: event
schedule:
  - { title: Morning Program, start: 2026-02-06T09:00:00, end: 2026-02-06T12:00:00, description: "Testing strategies for OpenFOAM and FAIR4RS principles.", location: "Pendulum 0.36" }
  - { title: Lunch, start: 2026-02-06T12:00:00, end: 2026-02-06T13:00:00, icon: food-fork-drink }
  - { title: Afternoon Program, start: 2026-02-06T13:00:00, end: 2026-02-06T17:30:00, description: "Benchmarking CFS simulations, Coupling OpenFOAM with Isogeetric Analysis, LES of turbulent iron powder burner, power burner, experimental and numerical flow field study.", location: "Pendulum 0.36" }
---

Interested in CFD with OpenFOAM? Join the next Dutch OpenFOAM User Group meeting on February 6, 2026 at the TU Eindhoven!

<!-- more -->

## Registration:

Morning program: [https://www.aanmelder.nl/phdworkshop](https://www.aanmelder.nl/phdworkshop){:target=_blank}

Afternoon program: [https://www.aanmelder.nl/openfoamafternoon](https://www.aanmelder.nl/openfoamafternoon){:target=_blank}

---

This program is a partnership with the [Delft University of Technology DCSE](https://www.tudelft.nl){:target=_blank}, [Eindhoven University of Technology](https://www.tue.nl){:target=_blank} & [TU/e Supercomputing Center for AI - HPC - Quantum](https://supercomputing.tue.nl){:target=_blank}.
