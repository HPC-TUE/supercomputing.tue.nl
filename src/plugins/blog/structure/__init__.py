...
from material.plugins.blog.structure import ExcerptTreeprocessor, View, Category
...
class Post(Page):
    ...
    def __init__(self, file: File, config: MkDocsConfig):
        ...
        with open(file.abs_src_path, encoding = "utf-8-sig") as f:
            ...
            # which we display to the speaker with a nice error message
        ...
        # Initialize authors and categories
        ...
        if "title" not in self.meta:
            self.meta["title"] = get_markdown_title(self.markdown)
...
class Excerpt(Page):
    ...
    def __init__(self, post: Post, config: MkDocsConfig, files: Files):
        ...
        # event plugin, as the `abs_url` property would be missing
        ...
        # Initialize speakers and categories - note that views usually contain
        # subsets of those lists, which is why we need to manage them here
        self.authors: list[Author] = []
        self.categories: list[Category] = []

        # Initialize content after separator - allow template speakers to render
...
def _patch(config: MkDocsConfig):
    ...
    # Make sure that the speaker did not add another instance of the table of
    ...
    config.mdx_configs["toc"] = {
        ...
        **{
            "anchorlink": False,       # Don't render headline as clickable
            "baselevel": 1,            # Render h1 as h2 and so forth
